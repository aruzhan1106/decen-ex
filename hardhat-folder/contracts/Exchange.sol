...
contract Exchange is ERC20 {
    address public peachTokenAddress;

    // Exchange is inheriting ERC20, because our exchange would keep track of Crypto Dev LP tokens
    constructor(address _Peachtoken) ERC20("Peach LP Token", "PLP") {
        require(_Peachtoken != address(0), "Token address passed is a null address");
        peachTokenAddress = _Peachtoken;
    }
    ...
    **/
    ...
    function addLiquidity(uint _amount) public payable returns (uint) {
        ...
        uint peachTokenReserve = getReserve();
        ERC20 peachToken = ERC20(peachTokenAddress);
        /*
            If the reserve is empty, intake any user supplied value for
            `Ether` and `Crypto Dev` tokens because there is no ratio currently
        */
        if(peachTokenReserve == 0) {
            ...
            */
            ...
            uint peachTokenAmount = (msg.value * peachTokenReserve)/(ethReserve);
            require(_amount >= peachTokenAmount, "Amount of tokens sent is less than the minimum tokens required");
            // transfer only (cryptoDevTokenAmount user can add) amount of `Crypto Dev tokens` from users account
            // to the contract
            peachToken.transferFrom(msg.sender, address(this), peachTokenAmount);
...
*/
    ...
    */
    function ethToPeachToken(uint _minTokens) public payable {
        ...
        ERC20(peachTokenAddress).transfer(msg.sender, tokensBought);
    }

        /**
    * Swaps CryptoDev Tokens for Eth
    */
    function peachTokenToEth(uint _tokensSold, uint _minEth) public {
        uint256 tokenReserve = getReserve();
        ...
        ERC20(peachTokenAddress).transferFrom(
            msg.sender,
            address(this),
            _tokensSold
        );
