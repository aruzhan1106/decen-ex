...
export const addLiquidity = async (
  ...
  try {
    ...
    let tx = await tokenContract.approve(
      EXCHANGE_CONTRACT_ADDRESS,
      addPeachAmountWei.toString()
    );
    await tx.wait();
    // After the contract has the approval, add the ether and cd tokens in the liquidity
    tx = await exchangeContract.addLiquidity(addPeachAmountWei, {
 ...
 */
export const calculatePeach = async (
  _addEther = "0",
  ...
  const peachTokenAmount = _addEtherAmountWei
    .mul(peachTokenReserve)
    .div(etherBalanceContract);
  return peachTokenAmount;
};