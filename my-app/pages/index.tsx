...
import { addLiquidity, calculatePeach } from "../utils/addLiquidity";
import {
  getPeachTokensBalance,
  getEtherBalance,
  getLPTokensBalance,
  getReserveOfPeachTokens,
...
export default function Home() {
  ...
  const [reservedPeach, setReservedPeach] = useState(zero);
  // Keeps track of the ether balance in the contract
  const [etherBalanceContract, setEtherBalanceContract] = useState(zero);
  // cdBalance is the amount of `CD` tokens help by the users account
  const [peachBalance, setPeachBalance] = useState(zero);
  ...
  const [addPeachTokens, setAddPeachTokens] = useState(zero);
  ...
  const [removePeach, setRemovePeach] = useState(zero);
  ...
  const getAmounts = async () => {
    try {
      ...
      const _peachBalance = await getPeachTokensBalance(provider, address);
      ...
      const _reservedPeach = await getReserveOfPeachTokens(provider);
      // Get the ether reserves in the contract
      const _ethBalanceContract = await getEtherBalance(provider, null, true);
      setEtherBalance(_ethBalance);
      setPeachBalance(_peachBalance);
      setLPBalance(_lpBalance);
      setReservedPeach(_reservedPeach);
      setReservedPeach(_reservedPeach);
  ...
  const _addLiquidity = async () => {
    try {
      ...
      if (!addPeachTokens.eq(zero) && !addEtherWei.eq(zero)) {
        const signer = await getProviderOrSigner(true);
        setLoading(true);
        // call the addLiquidity function from the utils folder
        await addLiquidity(signer, addPeachTokens, addEtherWei);
        setLoading(false);
        // Reinitialize the CD tokens
        setAddPeachTokens(zero);
  ...
  const _getTokensAfterRemove = async (_removeLPTokens) => {
    try {
      ...
      const peachTokenReserve = await getReserveOfPeachTokens(provider);
      // call the getTokensAfterRemove from the utils folder
      const { _removeEther, _removePeach } = await getTokensAfterRemove(
        provider,
        removeLPTokenWei,
        _ethBalance,
        peachTokenReserve
      ...
      return (
        <div>
          <div className={styles.description}>
            ...
            {utils.formatEther(peachBalance)} Peach Tokens
            <br />
            {utils.formatEther(ethBalance)} Ether
            <br />
            {utils.formatEther(lpBalance)} Peach LP tokens
          </div>
          <div>
            ...
            {utils.parseEther(reservedPeach.toString()).eq(zero) ? (
              <div>
                <input
                  type="number"
                ...
                <input
                  type="number"
                  placeholder="Amount of Peach tokens"
                  onChange={(e) =>
                    setAddPeachTokens(
              ...
              <div>
                <input
                  ...
                  onChange={async (e) => {
                    ...
                    const _addPeachTokens = await calculatePeach(
                      e.target.value || "0",
                      etherBalanceContract,
                      reservedPeach
                    );
                    setAddPeachTokens(_addPeachTokens);
                  }}
                  className={styles.input}
                />
      ...
      return (
        <div>
          ...
          >
            ...
            <option value="peachToken">Peach Token</option>
          </select>
          <br />
          <div className={styles.inputDiv}>
            {/* Convert the BigNumber to string using the formatEther function from ethers.js */}
